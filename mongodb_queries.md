...
db.flights.find({year: 2020 ,month_name : "MAY"
...
db.flights.find({ total_flights : { $gt : 500}
...
db.flights.find({ airport_name : { $in  : ["Porto", "Kaunas", "Treviso"]}
...
db.flights.aggregate([{
   $project : {
  _id: 0 , airport_code : 1 ,flight_date : 1, total_flights : {'total_volume': 1}
   } }])
